document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    let customerInfoResponse = await postRequestWithToken(
      "admin/piggyBox/post/customerInfo",
      { candidateId }
    );
    let transactionHistoriesResponse = await postRequestWithToken(
      "admin/piggyBox/post/transactionHistory",
      { candidateId }
    );
    ...
    if (customerInfoResponse.success) {
      ...
      document.getElementById("isRequestedClosure").innerText =
        piggyBox.piggyBalance.toFixed(2);
      document.getElementById("isActive").innerText =
        piggyBox.unclearedBalance.toFixed(2);
      document.getElementById("byReferralId").innerText =
        piggyBox.interestBalance.toFixed(2);
    ...
    if (transactionHistoriesResponse.success) {
      ...
      transactionHistoriesResponse.transactionHistories.forEach(
        (transaction, index) => {
  ...
  document
    .getElementById("deposite-funds-btn")
  ...
  document
    .getElementById("deduct-funds-btn")
    .addEventListener("click", async () => {
  ...
  document.querySelector(".btn-primary").addEventListener("click", async () => {
    ...
    try {
      const response = await postRequestWithToken(
        "admin/piggyBox/post/transactionHistory",
        { fromDate, toDate, limit, candidateId }
      );
      if (response) {
        const tbody = document.querySelector("table tbody");
        tbody.innerHTML = ""; // Clear existing rows
